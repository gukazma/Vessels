...
foreach ($tickRate in @(60, 120)) {
    Invoke-GodotCheck -Name "test-$tickRate" -EngineArguments @(
        ...
        '--', "--ticks=$tickRate")
    Invoke-GodotCheck -Name "combat-$tickRate" -EngineArguments @(
        '--fixed-fps', "$tickRate", '--script', 'res://tests/inventory_combat_test.gd',
        '--', "--ticks=$tickRate")
    Invoke-GodotCheck -Name "weapon-visual-$tickRate" -EngineArguments @(
        '--fixed-fps', "$tickRate", '--script', 'res://tests/weapon_visual_test.gd',
        '--', "--ticks=$tickRate")
}
